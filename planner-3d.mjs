...
export async function create3D(container, floor, onSelect = () => {}, onViewChange = () => {}) {
  ...
  const { Scene, PerspectiveCamera, WebGLRenderer, AmbientLight, DirectionalLight,
    ...
    Color, Vector3, BoxHelper } = await import('./assets/planner-vendor/three.module.js');
  ...
  renderer.domElement.setAttribute('aria-label', floor.name+' 3D 가구 배치 미리보기'); container.replaceChildren(renderer.domElement);
  ...
  const root = new Group(); scene.add(root); const itemMeshes = new Map(); const raycaster = new Raycaster(); const pointer = new Vector2(); let selected=null,selectionBox=null; let moved=false; let downX=0,downY=0;
  ...
  const clear = () => { root.traverse(o=>{o.geometry?.dispose();if(o.material){for(const m of Array.isArray(o.material)?o.material:[o.material])m.dispose();}});root.clear();itemMeshes.clear();selectionBox=null; };
  ...
  function rebuild() {
    ...
    (project.items || []).forEach(addItem);highlight();
    render();
  }
  function addItem(i) {
  ...
  function fit() { const cx=(bounds[0]+bounds[2])/2, cy=(bounds[1]+bounds[3])/2; const span=Math.max(bounds[2]-bounds[0],bounds[3]-bounds[1])*MM; const distance=span/(2*Math.tan(camera.fov*Math.PI/360)*Math.min(1,camera.aspect))*1.15;camera.position.set(0,distance*.75,distance*.8); camera.lookAt(...world(cx,cy).map((v,j)=>j===1?0:v)); controls.target.set(0,0,0); controls.update(); }
  function highlight(){if(selectionBox){root.remove(selectionBox);selectionBox.geometry.dispose();selectionBox.material.dispose();selectionBox=null;}const g=itemMeshes.get(selected);if(g){selectionBox=new BoxHelper(g,0x1767bf);root.add(selectionBox);}}
  function render(){ renderer.render(scene,camera);onViewChange(); }
  renderer.domElement.addEventListener('pointerdown', e=>{downX=e.clientX;downY=e.clientY;moved=false}); renderer.domElement.addEventListener('pointermove', e=>{if(Math.hypot(e.clientX-downX,e.clientY-downY)>6)moved=true}); renderer.domElement.addEventListener('pointerup', e=>{ if(moved)return; const r=renderer.domElement.getBoundingClientRect(); pointer.x=((e.clientX-r.left)/r.width)*2-1; pointer.y=-((e.clientY-r.top)/r.height)*2+1; raycaster.setFromCamera(pointer,camera); const hit=raycaster.intersectObjects([...itemMeshes.values()].flatMap(g=>g.children),true)[0]; onSelect(hit?.object?.userData?.itemId||null); });
